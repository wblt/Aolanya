//
//  AYLFinishVCViewController.swift
//  Meimeila
//
...
import UIKit
import SwiftyJSON
class AYLFinishVCViewController: DDBaseViewController {
	var lowerId:String?
	...
	init() {
		super.init(nibName: String.init(describing:String.init(describing: AYLFinishVCViewController.self)), bundle: nil)
	}
			...
			self.agentVm.subordinateShoppingArray.forEach({ (model) in
				if model.uid == self.lowerId {
					...
					for (item) in shopinfo {
						if item["orderState"].stringValue == "3" {
...
extension AYLFinishVCViewController:UITableViewDelegate{
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		vc.shopModel.orderState = model.orderState
		vc.lowerId = model.uid
		self.navigationController?.pushViewController(vc, animated: true);
	}
}

extension AYLFinishVCViewController:UITableViewDataSource{
