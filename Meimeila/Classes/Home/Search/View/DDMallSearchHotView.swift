...
@objc protocol DDMallSearchHotViewDlegate {
    func hotItemSelected(text: String)
}

class DDMallSearchHotView: UIView {

    weak var delegate: DDMallSearchHotViewDlegate?
    ...
    @objc func itemClick(tap: UITapGestureRecognizer) {
        ...
        let text = label.text!
        if let _ = delegate {
            delegate?.hotItemSelected(text: text)
        }
    }
    ...
    private lazy var iconImageView: UIImageView = {
        ...
        view.image = UIImage.init(named: "hot_search")
